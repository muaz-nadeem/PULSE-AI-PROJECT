...
"[project]/app/api/extension/settings/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        const { data: blockingData, error: blockingError } = await supabase.from("blocking_settings").select("*").eq("user_id", user.id).single();
        ...
        try {
            const { data } = await supabase.from("accepted_schedules").select("*").eq("user_id", user.id).eq("date", today).single();
        ...
        if (currentSchedule.length > 0) {
            isScheduledTime = currentSchedule.some((item)=>{
...
//# sourceMappingURL=%5Broot-of-the-server%5D__3f1fb329._.js.map