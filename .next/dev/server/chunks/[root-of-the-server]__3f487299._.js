...
"[project]/app/api/extension/tabs/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    try {
        ...
        const { data: existing } = await supabase.from("tab_analytics").select("*").eq("user_id", user.id).eq("domain", domain).eq("date", today).single();
        if (existing) {
            // Update existing record
            const { error } = await supabase.from("tab_analytics").update({
            ...
            // Insert new record
            const { data, error } = await supabase.from("tab_analytics").insert({
                user_id: user.id,
                domain,
                url: url || null,
                title: title || null,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__3f487299._.js.map