...
import { useAICoach, type ConversationMessage } from "../model/useAICoach"
...
// Convert hook's message format to component's format
function toMessage(msg: ConversationMessage): Message {
  return {
    id: msg.id,
    text: msg.content,
...
export default function CoachPanel() {
  ...
  const { messages: hookMessages, isLoading, error, sendMessage: hookSendMessage, refreshHistory } = useAICoach()
  const [input, setInput] = useState("")
  const [isLoadingHistory, setIsLoadingHistory] = useState(true)
  const [isMounted, setIsMounted] = useState(false)
  const messagesEndRef = useRef<HTMLDivElement>(null)

  // Convert hook messages to local format, add welcome message if empty
  const messages: Message[] = hookMessages.length > 0
    ? hookMessages.map(toMessage)
    : [{
      id: "welcome",
  ...
  // Mark as mounted and history loaded when hook has finished
  useEffect(() => {
    setIsMounted(true)
  }, [])

  useEffect(() => {
    if (hookMessages.length >= 0 && isLoadingHistory) {
      setIsLoadingHistory(false)
    }
  }, [hookMessages, isLoadingHistory])

  async function handleSend() {
    if (!input.trim() || isLoading) return
    const message = input.trim()
    setInput("")
    await hookSendMessage(message)
  }
